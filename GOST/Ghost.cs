...
namespace GOST
{
	class Ghost
	{
		...
		// xor соответствующих элементов каждой последоватлеьности
		...
		// замена байтов входной последовательности на значения из соответствующие значения таблицы с
		...
		// перестановка байтов в соответствии с правилами перестановки описанными в табличке тау
		...
		private byte[] G_n(byte[] N, byte[] h, byte[] m)
		{
			...
			byte[] t = E(K, m);
			t = Xor512(t, h);
			byte[] newh = Xor512(t, m);
			return newh;
		}
		private byte[] NumberToBytes(ulong number)
		{
		...
		public byte[] GetHash(byte[] message)
		{
			...
			byte[] N_0 = new byte[64]; // нулевой вектор
			byte[] N_512 = NumberToBytes(512); // 0x00000200

			int processedBytes = 0;

			// Шаг 1: Обработка ПОЛНОГО блока (512 бит = 64 байта)
			if (totalBits >= 512 && message.Length >= 64)
			{
				byte[] fullBlock = new byte[64];
				Array.Copy(message, 0, fullBlock, 0, 64);
				processedBytes += 64;

				h = G_n(N, h, fullBlock);
				N = SumMod512(N, N_512);
				Sigma = SumMod512(Sigma, fullBlock);
				totalBits -= 512;
			}

			// Шаг 2: Дополнение и обработка ОСТАТКА
			byte[] paddedBlock = new byte[64];
			int remainingBytes = message.Length - processedBytes;

			// ВАЖНО: Проверяем границы!
			if (remainingBytes > 0 && remainingBytes <= 64)
			{
				// Очищаем блок
				for (int i = 0; i < 64; i++)
				{
				...
				// Добавляем бит '1' в правильную позицию
				int padPosition = 64 - remainingBytes - 1;
				if (padPosition >= 0 && padPosition < 64)
				{
					paddedBlock[padPosition] = 0x01;
				}

				// Копируем остаток сообщения
				if (processedBytes >= 0 && processedBytes < message.Length)
				{
					int bytesToCopy = Math.Min(remainingBytes, message.Length - processedBytes);
					Array.Copy(message, processedBytes, paddedBlock, 64 - remainingBytes, bytesToCopy);
				}
			}
			else
			{
				// Если нет остатка (сообщение кратно 64 байтам)
				paddedBlock[63] = 0x01;
			}

			h = G_n(N, h, paddedBlock);
			byte[] mesLen = NumberToBytes((ulong)(remainingBytes * 8));
			N = SumMod512(N, mesLen);
			Sigma = SumMod512(Sigma, paddedBlock);

			// Шаг 3: Финальное преобразование
			h = G_n(N_0, h, N);
			h = G_n(N_0, h, Sigma);

			return (outLen == 512) ? h : h.Take(32).ToArray();
		}
	}
}
