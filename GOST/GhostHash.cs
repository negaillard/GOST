...
public class GhostHash
{
	...
	private const int BlockLen = 64; // 512 бит = 64 байта
	...
	private byte[] SumMod(byte[] a, byte[] b)
	{
		...
		if (b.Length < BlockLen)
		{
			...
			Array.Copy(b, 0, tmp, BlockLen - b.Length, b.Length);
			b = tmp;
		}
	...
	public byte[] GetHash(byte[] message, bool is512Bit = false)
	{
		int lent = message.Length * 8;
		byte[] h = new byte[BlockLen];

		byte[] N_0 = new byte[BlockLen]; // все нули
		byte[] N = new byte[BlockLen];
		byte[] Sigma = new byte[BlockLen];

		// Инициализация h в зависимости от размера хеша
		if (is512Bit)
		{
			// Для 512-бит: h = 00000000...
			Array.Fill(h, (byte)0x00);
		}
		else
		{
			// Для 256-бит: h = 01010101...
			Array.Fill(h, (byte)0x01);
		}

		Array.Fill(N, (byte)0x00);
		Array.Fill(Sigma, (byte)0x00);

		byte[] N_512 = NumberToBytes(512);

		int inc = 0;
		while (lent >= 512)
		{
			inc++;
			byte[] m = new byte[BlockLen];
			Array.Copy(message, message.Length - BlockLen * inc, m, 0, BlockLen);
			h = G_n(N, h, m);
			N = SumMod(N, N_512);
			Sigma = SumMod(Sigma, m);
			lent -= 512;
		}

		byte[] mLast = new byte[BlockLen];
		int ostLen = message.Length - inc * 64;
		Array.Copy(message, 0, mLast, BlockLen - ostLen, ostLen);
		mLast[BlockLen - ostLen - 1] = 0x01;

		h = G_n(N, h, mLast);
		byte[] mesLen = NumberToBytes((ulong)ostLen * 8);
		N = SumMod(N, mesLen);
		Sigma = SumMod(Sigma, mLast);

		h = G_n(N_0, h, N);
		h = G_n(N_0, h, Sigma);

		// Размер выхода в зависимости от версии
		byte[] res = is512Bit ? new byte[64] : new byte[32];
		Array.Copy(h, 0, res, 0, res.Length);
